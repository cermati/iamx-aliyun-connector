...
exports.AccountManager = class AccountManager {
  ...
  unbindMFADevice(ramUser) {
    return this._unbindMFADevice(ramUser);
  }
  ...
  _unbindMFADevice (ramUser) {
    return this.rpcClient().request('UnbindMFADevice', {
      UserName: ramUser.username
    }).catch((err) => {
      if (err.name == 'EntityNotExist.User.MFADeviceError') {
        return Promise.resolve(null)
      }
      throw(err)
    })
