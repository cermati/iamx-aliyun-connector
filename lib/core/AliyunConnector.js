...
exports.Connector = class AliyunConnector {
  ...
  async show(context) {
    ...
    let [policyResult, groupResult] = Promise.all([policiesPromise, groupsPromise]);
  ...
  async fetchBatch(context) {
    ...
    let results = await Promise.map(resp.Users.User, async (user) => {
      ...
      let [groupRes, policyRes] = await Promise.all([groupsPromise, policiesPromise]);

      let groups = groupRes.Groups.Group.map(g => g.GroupName);
      let userPolicies = policyRes.Policies.Policy.map(p => {
        return { name: p.PolicyName, type: p.PolicyType };
      });
